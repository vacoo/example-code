...
import * as Actions from '@resources/websocket/actions';
import { Action, Serializer } from '@resources/websocket/_types';
...
export class ReduxWebSocket {
    ...
    private handleClose = (dispatch: Dispatch, prefix: string, event: Event) => {
        dispatch(Actions.closed(event, prefix));
    ...
    private handleError = (dispatch: Dispatch, prefix: string) => {
        dispatch(Actions.error(null, new Error('`redux-websocket` error'), prefix));
    ...
    private handleOpen = (
        ...
        if (this.reconnectionInterval) {
            ...
            dispatch(Actions.reconnected(prefix));
        }
        ...
        dispatch(Actions.open(event, prefix));
     ...
     */
    private handleMessage = (dispatch: Dispatch, prefix: string, event: Event) => {
        dispatch(Actions.message(event, prefix));
    };
    ...
    private handleBrokenConnection = (dispatch: Dispatch) => {
        ...
        dispatch(Actions.broken(prefix));
        dispatch(Actions.beginReconnect(prefix));

        this.reconnectCount = 1;

        dispatch(Actions.reconnectAttempt(this.reconnectCount, prefix));

        // Attempt to reconnect immediately by calling connect with assertions
        // that the arguments conform to the types we expect.
        this.connect({ dispatch } as MiddlewareAPI, { payload: this.lastConnectPayload } as Action);
